...
%   with specified errors modified from codes written by Nils Sjoberg 
...
h=plot(x,y, 'o', 'markers',4 , 'LineWidth',5 ); % main plot
...
for k=1:length(x)
    if ~isempty(lx) & ~isempty(ly) % both errors are specified
        ...
        hold on;
        l2=line([lx(k) lx(k)],[y(k)-Border*erry(k) y(k)+Border*erry(k)]);
        l3=line([ux(k) ux(k)],[y(k)-Border*erry(k) y(k)+Border*erry(k)]);
        l4=line([x(k) x(k)],[ly(k) uy(k)]);
        l5=line([x(k)-Border*errx(k) x(k)+Border*errx(k)],[ly(k) ly(k)]);
        l6=line([x(k)-Border*errx(k) x(k)+Border*errx(k)],[uy(k) uy(k)]);
        allh(k, :)=[l1, l2, l3, l4, l5, l6];
    elseif isempty(lx) & ~isempty(ly) % x errors are not specified
        l4=line([x(k) x(k)],[ly(k) uy(k)]);
        hold on;
        errx=nanmean(abs(diff(x)));
        l5=line([x(k)-Border*errx x(k)+Border*errx],[ly(k) ly(k)]);
        l6=line([x(k)-Border*errx x(k)+Border*errx],[uy(k) uy(k)]);
        allh(k, 4:6)=[l4, l5, l6];
    elseif ~isempty(lx) & isempty(ly) % y errors are not specified
        l1=line([lx(k) ux(k)],[y(k) y(k)]);
        hold on;
        erry=nanmean(abs(diff(y)));
        l2=line([lx(k) lx(k)],[y(k)-Border*erry y(k)+Border*erry]);
        l3=line([ux(k) ux(k)],[y(k)-Border*erry y(k)+Border*erry]);
        allh(k, 1:3)=[l1, l2, l3];
    else % both errors are not specified
    end
    if exist('l1', 'var')
        h1=[l1, l2, l3]; % all handles
        set(h1, 'color', color, 'LineWidth',2 );
    end
    if exist('l4', 'var')
